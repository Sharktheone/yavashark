...
#[properties_new(raw)]
impl Reflect {
    pub fn apply(target: &Value, this: Value, args: Value, #[realm] realm: &mut Realm) -> ValueResult {
        let args = ArrayLike::new(args, realm)?.to_vec(realm)?;
        
        target.call(realm, args, this)
    }
    
    pub fn construct(target: &ObjectHandle, args: Value, new_target: &ObjectHandle, #[realm] realm: &mut Realm) -> ValueResult {
        if !new_target.is_constructor() {
            return Err(Error::ty_error(format!("{} is not a constructor", new_target.name())));
        }
        ...
        if let Some(proto) = new_target.resolve_property(&"prototype".into(), realm)? {
            val.as_object()?.define_property("__proto__".into(), proto)?;
        }
    ...
    pub fn get(target: ObjectHandle, prop: &Value, receiver: Option<Value>, #[realm] realm: &mut Realm) -> ValueResult {
    ...
    pub fn get_own_property_descriptor(target: &ObjectHandle, prop: &Value, #[realm] realm: &mut Realm) -> ValueResult {
        let obj = target.get();
