...
#[props(intrinsic_name = temporal_plain_date, to_string_tag = "Temporal.PlainDate")]
impl PlainDate {
    #[constructor]
    pub fn construct(
        year: i32,
    ...
    pub fn add(
        ...
        let opts = opts.and_then(|opts| opts.overflow).map(Into::into);
    ...
    pub fn with(
        ...
        let overflow = overflow.and_then(|opts| opts.overflow).map(Into::into);
