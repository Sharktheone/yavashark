use crate::builtins::intl::utils::{canonicalize_locale_list, get_option_bool, get_option_string, validate_currency_code};
...
impl NumberFormat {
    #[constructor]
    fn construct(locales: Option<Value>, options: Option<Value>, #[realm] realm: &mut Realm) -> Res<ObjectHandle> {
        let locales_list = canonicalize_locale_list(locales, realm)?;
        let locale = locales_list.get(0).cloned().unwrap_or_else(|| DEFAULT_LOCALE.to_string());
        ...
        let style = get_option_string(&options_obj, "style", &[STYLE_DECIMAL, STYLE_CURRENCY, STYLE_PERCENT], realm)?
            .unwrap_or_else(|| STYLE_DECIMAL.to_string());
 
        let currency = if style == STYLE_CURRENCY {
            let c = options_obj.get("currency", realm)?;
            if c.is_undefined() {
                return Err(Error::ty("currency option is required for style 'currency'"));
            }
        ...
        let use_grouping = get_option_string(&options_obj, "useGrouping", &[], realm)?.unwrap_or_else(|| "auto".to_string());
    ...
    pub fn supported_locales_of(locales: Value, options: Option<ObjectHandle>, #[realm] realm: &mut Realm) -> Res<ObjectHandle> {
        let list = canonicalize_locale_list(Some(locales), realm)?;
        let values = list.into_iter().map(|s| Value::String(s.into())).collect::<Vec<_>>();
