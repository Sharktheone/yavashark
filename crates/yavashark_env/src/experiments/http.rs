...
impl Http {
    ...
    fn server(ip: String, port: u16, callback: ObjectHandle, #[realm] realm: &mut Realm) -> crate::Result<()> {
        let mut server = SimpleHttpServer::new(ip, port, move |request| {
            let Ok(obj) = request.into_object(realm) else { return };
...
impl HttpRequest {
    fn into_object(self, realm: &Realm) -> crate::Result<ObjectHandle> {
        ...
        let headers = self.headers.into_iter().map(|(key, value)| {
            (key.into_value(), value.into_value())
        }).collect::<Vec<_>>();
...
impl<C: FnMut(HttpRequest)> SimpleHttpServer<C> {
    ...
    fn start(&mut self) -> io::Result<()> {
        ...
        for stream in listener.incoming() {
            ...
            let Some(mut request_line) = http_request.first().map(|x| x.split_whitespace()) else { continue };
            
            let Some(method) = request_line.next() else { continue };
            let Some(url) = request_line.next() else { continue };
            let headers = http_request.iter().skip(1).map(|x| {
