...
pub struct CharIteratorReceiver {
    pos: Position,
    buffer: *mut UnsafeBuffer, //mut, so there can't be multiple receivers
}
...
pub struct CharIteratorSender {
    buffer: *mut UnsafeBuffer,
}

impl Drop for CharIteratorSender {
    fn drop(&mut self) {
        unsafe { (*self.buffer).end.store(true, Ordering::Relaxed) };
        let res = unsafe {
            (*self.buffer).other_dropped.compare_exchange(
                false,
                true,
                Ordering::Relaxed,
                Ordering::Relaxed,
...
impl Drop for CharIteratorReceiver {
    fn drop(&mut self) {
        let res = unsafe {
            (*self.buffer).other_dropped.compare_exchange(
                false,
...
impl CharIterator {
    ...
    pub fn from_string(s: String) -> anyhow::Result<CharIteratorReceiver> {
        CharIteratorReceiver::try_from(s)
    }
}

impl TryFrom<String> for CharIteratorReceiver {
    type Error = anyhow::Error;
    fn try_from(s: String) -> Result<CharIteratorReceiver, Self::Error> {
        let len = s.len();
...
impl TryFrom<&str> for CharIteratorReceiver {
    type Error = anyhow::Error;
    fn try_from(s: &str) -> Result<CharIteratorReceiver, Self::Error> {
        let buffer = Box::new(UnsafeBuffer {
...
impl CharIteratorReceiver {
    ...
    fn skip_n(&mut self, n: u8) {
        ...
        loop {
            ...
            if read_pos == write_pos {
                if buffer.end.load(Ordering::Relaxed) {
                    return;
                } else {
                    std::hint::spin_loop();
                }
                ...
                buffer
                    .read_pos
                    .store((read_pos + n) % buffer.size, Ordering::Relaxed);
            }
        }
    }
    ...
    pub fn next_n<const N: usize>(&mut self) -> Option<NextN<N>> {
        ...
        let write_pos = buffer.write_pos.load(Ordering::Relaxed);
        if read_pos == write_pos && buffer.end.load(Ordering::Relaxed) {
            return None;
        }
        ...
        if buffer.size > N {
            ...
            let end_pos = (read_pos + N) % buffer.size;
            if end_pos > read_pos {
                ...
                loop {
                    ...
                    if write_pos > end_pos || write_pos < read_pos {
                        let next_buffer = {
                            let Some(buf) = buffer.buffer.get(read_pos..end_pos) else {
                        ...
                        return {
                            Some(NextN {
                                ...
                                consume: Some(Box::new(move || {
                                    buffer.read_pos.store(end_pos, Ordering::Relaxed);
                                })),
                            })
                        };
                    } else {
                        if buffer.end.load(Ordering::Relaxed) {
                            ...
                            let len = buffer.size - read_pos + end_pos;

                            let mut buf = vec![0; len].into_boxed_slice();

                            return if read_pos < write_pos {
                                let buf = &buffer.buffer[read_pos..write_pos];
                                ...
                                let elem_to_buffer_end = &buffer.buffer[read_pos..];
                                ...
                                // buffer.size - read_pos + end_pos = len = buf.len()
                                //^^^^^^^^^^^^^^^^^^^^^^^^^^^^ ↑
                                // elem_to_buffer_end.len()    PLUS, so there are more elements in buf than elem_to_buffer_end
                                // elem_to_buffer_end (or buffer.buffer) and buf will never overlap since we just allocated buf
                                unsafe {
                                ...
                                let elem_to_end_pos = &buffer.buffer[..end_pos];
                                ...
                                // because we have already copied buffer.size - read_pos elements to buf, and we have N - (buffer.size - read_pos) elements left
                                ...
                                // elem_to_end_pos (or buffer.buffer) and buf_to_end_pos will never overlap since we just allocated buf
                ...
                loop {
                    ...
                    if write_pos > end_pos && write_pos < read_pos {
                        ...
                        // `buf` is valid for `buf.len()` elements by definition, which is ALWAYS more than `elem_to_buffer_end.len()`,
                        // because we have already checked that `end_pos` is less than `read_pos` and `end_pos` is less than `write_pos`
                        // which means that buffer.size - read_pos + end_pos = N = buf.len()
                        //                 ^^^^^^^^^^^^^^^^^^^^^^^^^^^^ ↑
                        //             elem_to_buffer_end.len()        PLUS, so there are more elements in buf than elem_to_buffer_end
                        // elem_to_buffer_end (or buffer.buffer) and buf will never overlap since we just allocated buf
                        unsafe {
            ...
            let len = buffer.size - 1; //same as buffer.size - read_pos + end_pos
...
impl Iterator for CharIteratorReceiver {
    type Item = u8;
    ...
    fn next(&mut self) -> Option<u8> {
        ...
        loop {
            if read_pos == buffer.write_pos.load(Ordering::Relaxed) {
                ...
                let byte = buffer.buffer[read_pos];
                buffer
                    .read_pos
                    .store((read_pos + 1) % buffer.size, Ordering::Relaxed);
                return Some(byte);
            }
        }
    }
...
impl CharIteratorSender {
    ...
    fn push_with_pos(&mut self, byte: u8, write_pos: usize) {
        ...
        if write_pos == buffer.read_pos.load(Ordering::Relaxed) {
            if buffer.end.load(Ordering::Relaxed) {
                return;
            }
            ...
            buffer.buffer[write_pos] = byte;
            buffer
                .write_pos
                .store((write_pos + 1) % buffer.size, Ordering::Relaxed);
        }
    }
}
