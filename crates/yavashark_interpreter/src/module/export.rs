...
use swc_ecma_ast::{
    ...
    ImportSpecifier, ModuleExportName, NamedExport,
};
...
impl Interpreter {
    ...
    pub fn run_export_default_expr(
        realm: &mut Realm,
    ...
    pub fn run_export_default_decl(
        realm: &mut Realm,
    ...
    pub fn run_export_all(
        realm: &mut Realm,
        stmt: &ExportAll,
        scope: &mut ModuleScope,
    ) -> RuntimeResult {
        let module = Self::resolve_module(
            stmt.src.value.as_str(),
            stmt.with.as_deref(),
            &scope.scope.get_current_path()?,
            realm,
        )?;
