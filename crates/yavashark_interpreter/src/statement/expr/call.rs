...
impl Interpreter {
    ...
    pub fn run_call_expr(
        ...
        Ok((match expr {
            ...
            Expr::Invalid(stmt) => return Err(ControlFlow::error(format!(
                "{:?}: Invalid expression.",
                stmt.span
            ))),
            _ => return Err(ControlFlow::error(format!(
                "{span:?}: TS and JSX are not supported."
            ))),
        }, None))
    }
    
    pub fn run_call_member(ctx: &mut Context, stmt: &MemberExpr, scope: &mut Scope) -> Result<(Value, Value), ControlFlow> {
        let (value, par) = Self::run_call_expr(ctx, &stmt.obj, stmt.span, scope)?;

        Ok((Self::run_member_on(ctx, value.copy(), &stmt.prop, stmt.span, scope)?, par.unwrap_or(value)))
    }
}
