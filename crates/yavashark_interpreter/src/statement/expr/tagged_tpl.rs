...
impl Interpreter {
    pub fn run_tagged_tpl(realm: &mut Realm, stmt: &TaggedTpl, scope: &mut Scope) -> RuntimeResult {
        ...
        let mut exprs = stmt.tpl.exprs.iter().map(|e| Self::run_expr(realm, e, e.span(), scope)).collect::<Result<Vec<Value>, ControlFlow>>()?;

        let quasis = stmt.tpl.quasis.iter().map(|q| q.raw.to_string().into()).collect::<Vec<Value>>();
        ...
        Ok(tag.call(realm, args, scope.this()?) //In strict mode, this is undefined
            .map_err(|mut e| {
