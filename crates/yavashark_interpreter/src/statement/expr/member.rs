...
impl Context {
    pub fn run_member(&mut self, stmt: &MemberExpr, scope: &mut Scope) -> RuntimeResult {
        ...
        let name = match &stmt.prop {
            ...
            _ => return Err(ControlFlow::error("Unsupported member expression property".to_owned())),
        };
        ...
        match obj {
            ...
            _ => Err(ControlFlow::error("Member expression object is not an object".to_owned()))
        }
