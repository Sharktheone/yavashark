...
impl Context {
    pub fn run_for_of(&mut self, stmt: &ForOfStmt, scope: &mut Scope) -> RuntimeResult {
        ...
        match obj {
            Value::Object(obj) => {
                self.run_for_of_obj(&**obj.get()?, stmt, scope)
            }
            Value::Function(func) => {
                self.run_for_of_obj(func.get()?.as_object(), stmt, scope)
            }
            _ => {
    ...
    pub fn run_for_of_obj(&mut self, obj: &dyn Obj<Context>, stmt: &ForOfStmt, scope: &mut Scope) -> RuntimeResult {
