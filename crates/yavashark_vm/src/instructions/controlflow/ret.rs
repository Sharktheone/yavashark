...
pub fn return_acc(vm: &impl VM) -> ControlResult {
    let value = vm.acc();
    Err(ControlFlow::Return(value))
}

pub fn return_reg(reg: Reg, vm: &impl VM) -> ControlResult {
    let value = vm.get_register(reg)?;
    Err(ControlFlow::Return(value))
}

pub fn return_var(var: VarName, vm: &mut impl VM) -> ControlResult {
